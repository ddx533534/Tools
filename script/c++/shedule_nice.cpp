...
void check_parameters(int argc, char *argv[])
{
    if (argc < 3)
    {
        printf("You need input at least 2  extra parameters!");
        exit(EXIT_FAILURE);
    }
    int processTime = atoi(argv[1]);
    int processInterval = atoi(argv[2]);
    if (processTime < 1 || processInterval < 1)
    {
...
int main(int argc, char *argv[])
{
    ...
    puts("end estimation! ");
    pids = (pid_t *)malloc(PROCESS_COUNT * sizeof(pid_t));
    if (pids == NULL)
    {
    ...
    int i, ncreated;
    for (i = 0, ncreated = 0; i < PROCESS_COUNT; i++, ncreated++)
    {
        ...
        {
            nice(i == 0 ? MIDDLE_PRIORITY : LOW_PRIORITY);
